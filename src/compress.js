...
import { divide, splitPath } from '../utils/index.js'

let compressedMap = {}

/*判断是否压缩过*/
function isCompressed(path) {
    return Boolean(compressedMap[splitPath(path)]);
}
...
export async function compressSync(filesList, isCache, map, syncCount) {
    compressedMap = map
